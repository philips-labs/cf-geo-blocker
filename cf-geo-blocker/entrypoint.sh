...
#mv /usr/share/GeoLite2*/GeoLite2*.mmdb /usr/share/geoip/
...
echo "[$(date)]; Updating GeoIP configuration file"
mkdir -p  /usr/local/etc/
envsubst < /usr/local/share/GeoIP.conf.tmpl > /usr/local/etc/GeoIP.conf
